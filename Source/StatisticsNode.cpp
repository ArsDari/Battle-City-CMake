...
StatisticsNode::StatisticsNode(const TextureHolder &textures, const FontHolder &fonts, const World::Players &players, const std::vector<World::Spawnpoint> &enemies, bool isMultiplayer, int level) :
    mTileset(textures.get(Textures::IconTank)),
    mTileSize(mTileset.getSize()),
    mVertices(sf::PrimitiveType::Lines, enemies.size() * 4),
    mStatistics(textures.get(Textures::Statistics)),
    mPlayers(players),
    mEnemies(enemies),
    mNumOfTotalEnemies(enemies.size()),
    mIsMultiplayer(isMultiplayer),
    mFirstPlayerText(fonts.get(Fonts::Main), getLifesString(mPlayers[0].getPlayerData()), 8),
    mSecondPlayerText(fonts.get(Fonts::Main), getLifesString(mPlayers[1].getPlayerData()), 8),
    mLevel(fonts.get(Fonts::Main), setLevelString(level), 8)
{
...
void StatisticsNode::construct()
{
    ...
    {
        int x = i % 2;
        sf::Vertex *quad = &mVertices[static_cast<size_t>(i) * 4];
