...
static bool matchesCategories(SceneNode::Pair &colliders, Category type1, Category type2)
{
    ...
    if (static_cast<unsigned int>(type1) & static_cast<unsigned int>(category1) &&
        static_cast<unsigned int>(type2) & static_cast<unsigned int>(category2))
    {
        return true;
    }
...
void World::buildScene(int level)
{
    ...
    {
        Category category = (i == Entities) ? Category::Scene : Category::None;
        SceneNode::Ptr layer(new SceneNode(category));
        mSceneLayers[i] = layer.get();
        mSceneGraph.attachChild(std::move(layer));
    }
    mTilemap.load("Media/Maps/Map" + std::to_string(level) + ".txt");
    sf::Vector2f backgroundPosition{ 16, 8 };
    sf::Vector2f backgroundSize{ 208, 208 };
    sf::FloatRect backgroundRect{ backgroundPosition, backgroundSize };
    std::unique_ptr<RectangleNode> background(new RectangleNode(backgroundRect, sf::Color::Black));
...
void World::addEnemy()
{
    Tank::Type type = randomInt(8) == 0 ? Tank::Type::EnemyHeavyTank : static_cast<Tank::Type>(static_cast<unsigned int>(Tank::Type::EnemyLightTank) + randomInt(3));
    ...
    if (mLevelTime < sf::seconds(periodDuration))
    {
        ...
        command.action = derivedAction<Tank>([](Tank &tank, sf::Time) {
            auto randomRotation = [](Tank *tank) {
                Entity::Rotation rotation = static_cast<Entity::Rotation>(randomInt(static_cast<unsigned int>(Entity::Rotation::RotationCount)));
    ...
    else if (mLevelTime < sf::seconds(periodDuration * 2))
    {
        ...
        command.action = derivedAction<Tank>([this](Tank &tank, sf::Time) {
            auto rotateAfterPlayers = [this](Tank *tank) {
                Entity::Rotation rotation = Entity::Rotation::Down;
...
Entity::Rotation World::getRotationToTank(Tank *rhs, Tank *lhs) const
{
    ...
    {
        return randomInt(2) ? Entity::Rotation::Right : Entity::Rotation::Up;
    }
    // move right down
    else if (directionToGo.x >= 0 && directionToGo.y < 0)
    {
        return randomInt(2) ? Entity::Rotation::Right : Entity::Rotation::Down;
    }
    // move left down
    else if (directionToGo.x < 0 && directionToGo.y < 0)
    {
        return randomInt(2) ? Entity::Rotation::Left : Entity::Rotation::Down;
    }
    // move left up
    else
    {
        return randomInt(2) ? Entity::Rotation::Left : Entity::Rotation::Up;
    }
}
